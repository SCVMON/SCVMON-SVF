...
namespace SVF
{
    ...
    const llvm::cl::opt<std::string> Options::SrcFile(
        "source-file",  
        llvm::cl::init("sources.txt"),
        llvm::cl::value_desc("source file"),
        llvm::cl::desc("source file name for analysis")
    );

    const llvm::cl::opt<std::string> Options::SnkFile(
        "sink-file",  
        llvm::cl::init("sinks.txt"),
        llvm::cl::value_desc("sink file"),
        llvm::cl::desc("sink file name for analysis")
    );

    const llvm::cl::opt<std::string> Options::IncFile(
        "include-file",  
        llvm::cl::init("includes.txt"),
        llvm::cl::value_desc("include file"),
        llvm::cl::desc("include file name for analysis")
    );

    const llvm::cl::opt<std::string> Options::AddEdgeFile(
        "add-edge-file",  
        llvm::cl::init("add_edge.txt"),
        llvm::cl::value_desc("add edge file"),
        llvm::cl::desc("add edge file name")
    );

    const llvm::cl::opt<std::string> Options::DebugOnly(
        "debug-only",  
        llvm::cl::init(""),
        llvm::cl::value_desc("debug only option of LLVM"),
        llvm::cl::desc("debug only option of LLVM")
    );
} // namespace SVF.
